...
- **`secrets` runs the same script as the pre-commit hook**, `secret-scan.sh`, in its
  ...
  gate a moving target); the official `gitleaks-action` is deliberately not used, because it
  needs a paid licence for repositories owned by an organisation, while the binary itself is free.
- **The contract drift check runs the same script as the pre-commit hook**, `codegen.sh`
  (F1.5), with the hook in `stage` mode and the `static` job in `check` mode. Both regenerate
  ...
  its sources are not all staged it refuses the commit instead of guessing. That is the same
  failure CI would report, an hour earlier. The check compares with `git status` rather than
  `git diff --exit-code`, because `diff` only compares tracked content: a file the generator has
  only just started emitting, which is what a generator bump does, would slip past it untracked
  and unnoticed. The step sits last in `static` on purpose. `pnpm typecheck` above already pulled
...
- Each job reinstalls dependencies. That is the price of running them in parallel, and a
  ...
  composite action, because Dependabot scans `.github/workflows`: an action under
...
- DB credentials are declared once in the workflow env (`POSTGRES_*`). They configure
  ...
  DB-backed job (into `$GITHUB_ENV`), because the `env` context is unavailable in workflow- and
...
- **`build` is not covered by typecheck:** `tsc --noEmit` only reads the sources, while
  ...
  building the API was Playwright, and Playwright is skipped whenever the Clerk secrets are
  absent, i.e. on every Dependabot pull request.
...
- **The pre-commit hook runs the same checks in a fixed order**: secret scan, contract
  regeneration, lint-staged, `lint:docs`, typecheck. The order is not arbitrary. The scan
...
- **Installing the browser is the slowest step in that job, and it has two halves.** The
  ...
  when the loop is tightest, would never populate it. The system packages cannot be cached,
  because they are apt packages and each run gets a fresh VM. So the system dependencies are
  installed on every run, `install --with-deps` on a cache miss and `install-deps` on a hit,
  and that is not ceremony: nine of the libraries
...
- **`sonar`, the SonarQube Cloud analysis** (F1.12): cognitive complexity, cross-workspace
  duplication, security hotspots, test coverage and a quality gate on **new** code. Those are
  the signals ESLint, Prettier and `strict` TypeScript don't carry. Scanner configuration
  ...
  history (`fetch-depth: 0`, since Sonar attributes issues to new code via blame) and provisions
  ...
  from already-merged commits, no single commit can move it, and both Railway
  services deploy with Wait for CI (see [deploy-railway.md](deploy-railway.md)), so a red gate
  ...
  still turns `main` red and still holds the deployment. That is deliberate, and for the same
  ...
  recognising for what it is when a run fails, because an outage reads exactly like a code
  problem.
...
- **Require a pull request before merging**, with zero required approvals, since there is one
  maintainer;
