...
deploys to dev automatically; a pre-deploy command applies the Prisma migrations.
Production is separate (Phase 10).

## How it works

- **Images are Docker** (not Nixpacks): [apps/api/Dockerfile](../apps/api/Dockerfile) and
  [apps/web/Dockerfile](../apps/web/Dockerfile), with the repository root
  (pnpm-workspace) as the build context. Build and deploy settings are config-as-code, in
  [apps/api/railway.json](../apps/api/railway.json) and [apps/web/railway.json](../apps/web/railway.json),
...
- **api** is multi-stage. Prod dependencies are installed as a separate layer
  ...
  migrations, and the pre-deploy command from railway.json runs them (shell-independent:
  `node .../prisma/build/index.js
migrate deploy --config packages/db/prisma.config.ts`). That is why `prisma` and `dotenv` are
  in `dependencies` of the `@rondo/db` package, not in dev.
- **web** uses Next.js `output: 'standalone'` (see next.config.ts), so the runner gets a
...
- **Local development is unchanged.** `docker-compose.yml` still starts only
  ...
  slow, so the images are for Railway only).

## One-time project setup in Railway

1. **New Project** → `rondo-money` (the live one is named after the product; its
   environment is `development`, and production gets its own in Phase 10); inside it,
...
2. **New → GitHub Repo** → this repository, service `api`:
   ...
   - Variables: `DATABASE_URL` = `${{Postgres.DATABASE_URL}}`, `WEB_ORIGIN` = the web URL
     ...
     but produces something no other tool accepts as a PEM, so don't.
     (In `apps/api/.env.local` the `\n` form _is_ the correct one: dotenv expands it inside
     double quotes, and the api sees real newlines.) Don't set `PORT`, since Railway injects
     its own;
     ...
     **`NODE_ENV`** (F1.4) decides whether the API serves its own documentation at `/docs`,
     and it does everywhere except production. The image sets `NODE_ENV=production` itself
     ([apps/api/Dockerfile](../apps/api/Dockerfile)), which is the safe default, because a new
     ...
     boot rather than guess which side of that line it is on. The HTTP adapter mounts the docs
     route, so it is not behind the auth guard: wherever it is on, it is on for
     everyone;
...
Directory is set. Everything not listed deliberately does **not** redeploy: `.claude/**`,
`docs/**`, `.github/**`, the top-level prose. Those files do land in a build layer
(`COPY . .`), but never in the runner image, so rebuilding both images for a documentation
commit buys nothing.
...
empty**. That is one source of truth, and it moves with the repository. Should a service ever stop
...
environment is renamed, and it leaks the environment name into the URL. You add a custom
domain per service (Settings → Networking → Custom Domain, or `railway domain <name>`);
...
- `WEB_ORIGIN` must be the **exact** origin, scheme included (`https://dev.<domain>`), with no
  ...
  A browser fails the CORS check before it ever sees a response, and a direct API client gets
  **401** from token verification. Meanwhile `/health` stays green either way, because the
...
| api     | `WEB_ORIGIN`                        | the public web URL (exact, for CORS + `azp`) | runtime (start fails without)               |
...
- a commit touching only `packages/db` redeploys **both** services, since `/packages/**` is
  watched by each, and the api logs show the pre-deploy migration; a commit touching only
  `docs/` or `.claude/` redeploys nothing (watch paths).
