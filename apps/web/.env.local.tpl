# The env contract of @ffai/web (single source — there is no separate .env.example).
...
#
# Secrets live in a single 1Password item: vault "fin-flow", item "local", field name =
...
#   op item edit local --vault fin-flow \
#     'NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY[text]=pk_test_...' \
#     'CLERK_SECRET_KEY[concealed]=sk_test_...'

# Base URL of the @ffai/api backend. Exposed to the browser, so it must be NEXT_PUBLIC_*.
...
NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY="{{ op://fin-flow/local/NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY }}"
CLERK_SECRET_KEY="{{ op://fin-flow/local/CLERK_SECRET_KEY }}"
