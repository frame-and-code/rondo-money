...
transactions and monthly assignments, and a read of what a budget owns is scoped to the
caller's active budget.
...
so it can be corrected while the account is still empty and every balance is summed on demand.
An account that has been emptied is archived for good, and keeps a history that stays readable. A month of the budget can be read and worked in:
