...
/**
 * Erase everything the caller owns
 *
 * Deletes every row belonging to the caller in one database transaction: budgets, accounts, records, envelopes, goals and their own settings. It is physical deletion and there is nothing to undo it with. The account itself is untouched, because this API holds no credential that could reach the identity provider; a client that wants the account gone deletes it with the session it already has. It is a POST because the idempotency key travels in the body.
 */
export const meControllerEraseMutation = (options?: Partial<Options<MeControllerEraseData>>): UseMutationOptions<MeControllerEraseResponse, MeControllerEraseError, Options<MeControllerEraseData>> => {
    const mutationOptions: UseMutationOptions<MeControllerEraseResponse, MeControllerEraseError, Options<MeControllerEraseData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await meControllerErase({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
 ...
 *
 * Writes the language onto the settings of the caller, creating the row when they have never read it. The theme is not here on purpose: it belongs to the device rather than to the account, so it never reaches the server.
 */
export const userSettingsControllerUpdateMutation = (options?: Partial<Options<UserSettingsControllerUpdateData>>): UseMutationOptions<UserSettingsControllerUpdateResponse, UserSettingsControllerUpdateError, Options<UserSettingsControllerUpdateData>> => {
    const mutationOptions: UseMutationOptions<UserSettingsControllerUpdateResponse, UserSettingsControllerUpdateError, Options<UserSettingsControllerUpdateData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await userSettingsControllerUpdate({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
...
/**
 * Settle an account against what it really holds
 *
 * Takes the balance the account really carries and writes the one record that makes the book agree with it: a correction for the difference, carrying no category, so the money lands in or leaves Ready to Assign the way an income with no envelope does. The balance is summed inside the same transaction that writes the correction, so a record arriving at the same moment cannot be counted twice. Balances that already agree are a success that writes nothing. The correction is dated today in the budget timezone, which is what a reconciliation means, and it is an ordinary record from then on.
 */
export const accountsControllerReconcileMutation = (options?: Partial<Options<AccountsControllerReconcileData>>): UseMutationOptions<AccountsControllerReconcileResponse, AccountsControllerReconcileError, Options<AccountsControllerReconcileData>> => {
    const mutationOptions: UseMutationOptions<AccountsControllerReconcileResponse, AccountsControllerReconcileError, Options<AccountsControllerReconcileData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await accountsControllerReconcile({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
...
/**
 * Mark a category paid for one month
 *
 * Puts a mark on the pair of category and month saying the payment the category stands for is done. The mark is a state the user sets rather than a number the app derives, and it changes no amount. The next month starts without one. Marking a category that already carries the mark leaves it as it is.
 */
export const categoryPaidControllerMarkMutation = (options?: Partial<Options<CategoryPaidControllerMarkData>>): UseMutationOptions<CategoryPaidControllerMarkResponse, CategoryPaidControllerMarkError, Options<CategoryPaidControllerMarkData>> => {
    const mutationOptions: UseMutationOptions<CategoryPaidControllerMarkResponse, CategoryPaidControllerMarkError, Options<CategoryPaidControllerMarkData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await categoryPaidControllerMark({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
...
/**
 * Take the paid mark off a category for one month
 *
 * Removes the mark from the pair of category and month. A category carrying none is left as it is.
 */
export const categoryPaidControllerUnmarkMutation = (options?: Partial<Options<CategoryPaidControllerUnmarkData>>): UseMutationOptions<CategoryPaidControllerUnmarkResponse, CategoryPaidControllerUnmarkError, Options<CategoryPaidControllerUnmarkData>> => {
    const mutationOptions: UseMutationOptions<CategoryPaidControllerUnmarkResponse, CategoryPaidControllerUnmarkError, Options<CategoryPaidControllerUnmarkData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await categoryPaidControllerUnmark({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
