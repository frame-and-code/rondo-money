...
- **No decisions in code.** Why an approach was chosen, what was rejected, what a measurement
  showed, which ADR applies — that belongs to the Notion ticket that decided it. Not a
...
- Three places sit outside this, each for a mechanical reason: `///` in `schema.prisma`, which
  ...
  charter — what it refuses and what it knowingly does not — has nowhere else to live.
