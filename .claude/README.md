...
than fight a dependency that writes into the agent's context, we made the file ours. Next
...
| `/prep-pr <F1.x>`      | tidies, gates, sweeps the docs, reviews in rounds and fixes what blocks, then commits, pushes and opens the PR; a third round still blocking stops it before any git action                                           |
...
| [`stop-docs-drift.sh`](hooks/stop-docs-drift.sh)             | `Stop`                 | per area: names the document a changed area describes when that document did not change too, and falls back to a general nudge when the session touched no `.md` at all                                                                                             |
...
[`hooks.test.sh`](hooks/hooks.test.sh) runs as `pnpm test:hooks`, and it is a step of the CI
`unit` job. The two blocking guards decide by inspecting a command, so their failure mode is
silence. A rule that stops matching still exits 0 and the command goes through. That is why
they have cases and why a new blocking hook lands with its own.
...
blocked command into an allowed one. That file's own header owns the boundary and lists what
it deliberately does not do: expand variables, follow a command into another interpreter.

The cases are adversarial on purpose: every wrapper (`sudo`, `env`, `command`, an inline
assignment), every route round the secret scan, every refspec form that names main. Just as
load-bearing are the ordinary commands that must stay allowed, because a guard that blocks
real work gets switched off. `guard-db.sh` also has to refuse **without** echoing the
...
check that quietly stops matching is worse than none. That is the same reasoning by which
ADR-005 rejected "extension only, no rules for raw SQL".
...
`Bash(pnpm *)` the same way a deny would, so nothing fetches and executes a package
unattended, four reviewers spawned with no user turn between them least of all. The
legitimate use stays one confirmation away. Reach for `ask` whenever the answer is "not
without me" rather than "never". That is the mechanical twin of the top rule in `CLAUDE.md`,
and the reason a guard hook for them would be redundant.
...
[`docs-ownership.json`](config/docs-ownership.json) names which document owns which fact, plus
the prose patterns that are refused outright. `pnpm lint:docs`
([`check-docs.mjs`](../check-docs.mjs)) enforces it as a gate step; the rule it serves is
[`specs.md`](rules/specs.md).

[`external-docs.json`](config/external-docs.json) lists the external documentation this
