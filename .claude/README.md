...
| `/close-ticket <F1.x>` | after the merge: ticks the ticket's AC/DoD with evidence, corrects what the work made false, records the PRs and the decisions, puts ✅ in the title, returns to a fresh `main` and deletes the merged branches (local and remote) |
