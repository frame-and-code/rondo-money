---
description: Turn a ticket into an ordered, file-scoped implementation plan. Plans only, and writes no code.
...
has to invent structure. **This command writes no code.** The plan is the deliverable.
...
1. **Read the ticket.** For an `F<x>.<y>` argument, pull the Notion ticket (through the
   ...
   and do not reconstruct a ticket from memory) and read its Goal, Steps and Acceptance
...
Steps are file-scoped and numbered. A step like "update the API" is not one. Stop after
presenting the plan. Implementation starts when the user says so.
